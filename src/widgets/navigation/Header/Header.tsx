    ...
    return <nav className=" bg-background flex w-full justify-around items-center h-20 gap-x-400">
        ...
        <div className='flex flex-row items-center gap-2 mr-2'>
            ...
            <div onClick={() => setActive(!isActive)}>
                <svg width="14" height="9" viewBox="0 0 14 9" fill="none" xmlns="http://www.w3.org/2000/svg" className={`max-w-6 max-h-6 cursor-pointer ${isActive ? 'rotate-180' : ''}`}>
                    <path d="M13 1L7 7L1 1" stroke="#525252" stroke-width="1.5" />
                </svg>
                <div className={`flex flex-col absolute items-end justify-center z-10 w-50 h-15 right-3 top-15 shadow-lg rounded-2xl overflow-hidden transition-all duration-200 ${isActive
                        ? 'opacity-100 translate-y-0 pointer-events-auto'
                        : 'opacity-0 -translate-y-2 pointer-events-none'
                    }`}>
                    <button className=" cursor-pointer font-Montserrat mr-[12px]">Войти</button>
                    <button className=" cursor-pointer font-Montserrat mr-[12px]">Зарегистрироваться</button>
                </div>
